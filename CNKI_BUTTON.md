...
This fork keeps Zotero Connector's existing detection, translation, attachment, snapshot, and save behavior unchanged. It adds a page save button on both CNKI search-result pages and article detail pages.
...
1. Build the Manifest V3 extension with `./build.sh -p b -v 5.0.212.3`.
...
The content script runs only on `http://*.cnki.net/*` and `https://*.cnki.net/*`. It renders the button on CNKI search-result pages (`/defaultresult/`, `/search?`, `/AdvSearch?`) where clicking pops up item selection, and on article detail paths (`/kcms2/article/abstract`, `/kcms/detail/detail.aspx`) where clicking directly saves the record. Bare homepage, login, and security verification pages are excluded.
